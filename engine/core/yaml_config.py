...
class YAMLConfig(BaseConfig):
    ...
    def get_rank_batch_size(cfg):
        ...
        if total_batch_size is None:
            ...
            world_sz = dist_utils.get_world_size()
            assert (
                total_batch_size % world_sz == 0
            ), f"total_batch_size {total_batch_size} should be divisible by world size {world_sz}"
            bs = total_batch_size // world_sz
        return bs
